...
| D10 | **Verdict gains a fourth arm**: `no-verdict(cause)`, in a different type position than the verdicts proper. Abstractly the judge is total (three-valued); partiality is introduced by lowering. | "Ambiguous under the criterion" and "could not reach a verdict" demand different policy responses; folding them forces policies to spelunk findings. | **Implemented** (R3, tools/ADR-0010; §II.5) |
...
| O1 | **Verdict value names**: keep `:pass / :rejected` (act-flavored but serialized everywhere) or rename to verdict-layer words (`:conform / :violate`)? | Renaming is the one change touching serialized reports and both integration suites. | **Still open** — unchanged by R3; `:indeterminate`'s own rename to `:ambiguous` joins this question at signature-format v2 rather than being decided piecemeal (§II.5) |
| O2 | **`no-verdict` cause taxonomy**: minimum viable set, and whether `terminology-suppressed` (gate.fhir's former `:indeterminate`) is `no-verdict` or `ambiguous`. | First live specimen of the D10 split; this doc argued no-verdict (§II.5). | **Resolved** (R3, tools/ADR-0010): `no-verdict(:terminology-suppressed)` — the judge failed to fully apply the criterion; the criterion didn't fail to decide. `:indeterminate` is now reserved, no producer anywhere in this repo. Taxonomy stays minimum-viable (one member); grows as new operational-partiality causes are identified |
| O3 | **Routing-by-verdict: in the algebra or above it?** Sum-type outputs as a first-class construct vs. gates as workflow-level constructs calling algebra-level diagrams. | The union-resource construct answers the input side; the output side is untested. Adjudicated by re-expressing the pipeline (plan step 2), not on paper. | **Open** — `worst-of`'s rank order (§II.5) behaved as policy under real-data pressure during R3 (had to be revised once measured against a real corpus), which is evidence, not adjudication: a candidate for migrating out of the judge and into the policy layer at signature-format v2, rather than staying a judge-internal constant |
...
**Implemented (R3, tools/ADR-0010).** Code now carries `:pass / :rejected / :indeterminate / :no-verdict`, with `worst-of` ordering `:rejected > :no-verdict > :indeterminate > :pass` and empty ⇒ `:pass`. `judge.fhir`'s former `:indeterminate` case (terminology-suppressed) now emits `no-verdict(:terminology-suppressed)`; `:indeterminate` is reserved, no producer anywhere in this repo; `judge.v2` still never produces either.

- **O1** — verdict names. **Still open, unchanged by R3.** `:pass/:rejected` are act-flavored words on verdict values, but they're serialized in reports and asserted in both integration suites; deferred to signature-format v2, alongside `:indeterminate`'s own eventual rename to `:ambiguous`.
- **O2** — resolved as this design argued: `no-verdict(:terminology-suppressed)`, not `ambiguous` — the judge failed to fully *apply* the criterion; the criterion didn't fail to decide.
- **worst-of's ranking, revised from this design's first draft.** §II.5's original text (and tools/ADR-0010's first draft) ranked `no-verdict` above `rejected` outright — "a corpus that couldn't be fully judged is not a corpus that passed." R3's Step 5 integration run (the real validator, against real Synthea output) found this made every real, US-Core-profiled file's aggregate verdict `:no-verdict` regardless of an actual injected defect: such files always mix terminology-suppressed findings with genuine profile-driven violations in the same file (EXP-C5) — confirmed directly, one mutant bundle carried 3062 terminology-suppressed issues alongside its own defect. That overrode contract-pairing's and baseline-gating's polarity regression rather than merely losing to it. Revised ranking: `no-verdict` still beats `indeterminate`/`pass` (a corpus the judge couldn't fully apply its criterion to is worse than one the criterion simply didn't decide), but `rejected` — a confirmed violation — still dominates the aggregate. Still policy-flavored, still an O3-adjacent exhibit, now empirically informed rather than argued on paper alone.
