...
| D10 | **Verdict gains a fourth arm**: `no-verdict(cause)`, in a different type position than the verdicts proper. Abstractly the judge is total (three-valued); partiality is introduced by lowering. | "Ambiguous under the criterion" and "could not reach a verdict" demand different policy responses; folding them forces policies to spelunk findings. | **Implemented** (R3, ADR-0010; §II.5) |
...
**Implemented (R3, ADR-0010).** Code now carries `:pass / :rejected / :indeterminate / :no-verdict`, with `worst-of` ordering `:rejected > :no-verdict > :indeterminate > :pass` and empty ⇒ `:pass`. `judge.fhir`'s former `:indeterminate` case (terminology-suppressed) now emits `no-verdict(:terminology-suppressed)`; `:indeterminate` is reserved, no producer anywhere in this repo; `judge.v2` still never produces either.
