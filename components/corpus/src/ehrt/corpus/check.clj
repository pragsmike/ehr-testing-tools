(ns ehrt.corpus.check
  ...
  ehrt.corpus.check.schemas' registry). No arbitrary predicate
  ...
  canonicalizer list `cs` (ehrt.kernel.canonical) -- equivalence
  ...
  ehrt.corpus.diff (promoted from corpus.mutate's own diffing,
  now a second real consumer); unpaired files are reported as
  missing/extra findings.

  Report: reuses ehrt.judge.report's aggregation verbatim
...
(defn- canon-value
  ...
  (ehrt.kernel.interface/apply-canonicalizers)."
  [content steps]
  (if (empty? steps)
...
(defn- resolve-locator-value
  ...
  grammar (ehrt.kernel.interface/fhir-data-path) -- the same
...
(defn assertion-findings
  ...
  or a seq of findings (ehrt.judge.finding) when it
...
(defn check-corpus
  ...
  rejected -- report is a ehrt.judge.report Report, built
