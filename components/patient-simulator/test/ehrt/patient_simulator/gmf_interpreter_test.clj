(ns ehrt.patient-simulator.gmf-interpreter-test
  "Red tests for the GMF interpreter core (M5a Task 2, docs/gmf-
  interpreter.md sections 1-2 and 6) -- written before
  ehrt.patient-simulator.gmf-interpreter exists (sim/ADR-0004 test-first). Pure,
...
;; the table (`resources/patient-simulator/wellness-cadence.edn`'s own
...
(def wave-i-vital-sign-names
  ["Weight" "Heart Rate" "Respiration Rate" "Head Circumference" "Head Circumference Percentile"
   ...
   ;; at all (`patient-simulator/vital-signs.edn`'s own dated note has the
