...
- **History phase** (DOB → registration instant): the walk fast-forwards
  ...
  pathway step (`ehrt.patient-simulator.compile-trajectory`'s own
...
**`CompileTrajectory`** (`ehrt.patient-simulator.compile-trajectory`) turns
...
**`InjectChurn`** (`ehrt.sim-engine.churn`) is an IR→IR transform:
...
(`ehrt.sim-engine.engine/pop-min`) — every patient's own queued steps,
...
(`ehrt.sim-engine.engine`'s own `:discharge` method, scanning `world`'s
...
%% ACCEPTED AS DELIBERATELY HAND-OWNED, 2026-08-19 (ADR-0158, author
%% ruling R4-Q5 (d), review-4 register row D5-2). The five
%% docs/manual/assets/*.svg beside it in that row got a staleness
%% tripwire instead (hand-owned-assets.edn); this block did not,
%% because it draws pipeline SHAPE at a level that rarely moves and
%% names no single line-level source a tripwire could watch. Recorded
%% here so the next review stops re-finding it as an unwatched surface.
flowchart TD
