...
| Hidden, always-on Java lifecycle modules | No hidden modules: every module this project ever runs is an explicit, listable load (`ehrt.patient-simulator.gmf/loaded-modules`) — [`sim-theory.md`](../../sim/docs/sim-theory.md)'s own IR-transforms-as-composition-layer corollary, restated at the M5 roadmap entry |
...
`ehrt.patient-simulator.gmf/load-module` rejects a module on the bare
...
`ehrt.patient-simulator.gmf`'s own recognized state-type set at M5b; `Death`
...
compile-time truncation** — `ehrt.patient-simulator.compile-trajectory`'s own
...
**What it is.** `ehrt.patient-simulator.gmf/load-module` stops rejecting a
...
**Cost class:** loader-local. Touches `ehrt.patient-simulator.gmf`'s own
...
`ehrt.patient-simulator.gmf-interpreter` (a new `:death` case in `step`),
`ehrt.patient-simulator.compile-trajectory` (mapping a death trajectory event
...
age arithmetic
(`ehrt.patient-simulator.gmf-interpreter`'s own `epoch-day` clock,
...
own** namespace (`ehrt.patient-simulator.gmf`'s existing per-module namespacing
