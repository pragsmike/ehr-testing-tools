(ns ehrt.patient-simulator.census
  "GMF census tool (parity plan `.agents/plans/2026-08-02-gmf-parity-plan.md`
  §3, ADR-0031 AR-1, ADR-0034). A `patient-simulator` DEV ENTRY POINT, not a
  ...
  `components/patient-simulator`'s own product interface should carry, so
  ...
  Invocation: `clojure -M:dev -m ehrt.patient-simulator.census
  <synthea-checkout-dir> <out-dir>` from the workspace root (unchanged
  by the move -- the `:dev` alias already wires `poly/patient-simulator`
  ...
  `:load-failed` (`ehrt.patient-simulator.gmf/load-closure` itself
...
(defn census-one
  ...
  a `:rejected` Result -- a genuine `ehrt.patient-simulator.gmf` robustness
...
(defn -main
  "`clojure -M:dev -m ehrt.patient-simulator.census <synthea-checkout-dir>
