(ns ehrt.patient-simulator.gmf-interpreter
  ...
  two-phase run (Task 3, `ehrt.patient-simulator.gmf-interpreter.horizon` or
  ...
  module, `ehrt.patient-simulator.gmf/load-closure`'s own return shape),
...
;; sign's own raw name string -- "mapped alongside" `patient-simulator/
...
(def ^:private vital-sign-baseline-table
  (edn/read-string (slurp (io/resource "patient-simulator/vital-sign-baselines.edn"))))
...
(def ^:private vital-sign-reference-table
  (edn/read-string (slurp (io/resource "patient-simulator/vital-signs.edn"))))
...
(defn- validate-vital-sign-name
  ...
  (when-not (contains? vital-sign-reference-table vital-sign-name)
    (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: unrecognized vital-sign name -- not in patient-simulator/vital-signs.edn"
...
;; grounded, `resources/patient-simulator/wellness-cadence.edn`'s own
...
(def ^:private wellness-cadence-table
  (edn/read-string (slurp (io/resource "patient-simulator/wellness-cadence.edn"))))
...
;; sampling -- ehrt.patient-simulator.gmf's own normalized SampledDistribution
...
(defn- sample-distribution
  "ADR-0035 AR-1/AR-3: samples `ehrt.patient-simulator.gmf`'s own normalized
...
(defn- honest-absence
  ...
  (ex-info (str "ehrt.patient-simulator.gmf-interpreter: " (name condition-type)
...
(defn evaluate-condition
  ...
  (case (:condition-type condition)
    ...
    (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: unsupported condition type"
...
(defn- lookup-table-row-matches?
  ...
  `row` (`ehrt.patient-simulator.gmf/parse-lookup-table`'s own shape) match
  ...
  `:age-range` already performs, `ehrt.patient-simulator.gmf/parse-time-
...
(defn- resolve-lookup-table-transition
  ...
  state's own declared `:lookup-table-name` (`ehrt.patient-simulator.gmf/
...
(defn- resolve-transition
  ...
  above) -- the only kind that consults `tables` (`ehrt.patient-simulator.
...
(defn- open-encounter-index
  ...
  position (`ehrt.patient-simulator.compile-trajectory`'s own
...
(defn- emit-and-advance
  ...
  `:duration` (`ehrt.patient-simulator.gmf`'s own `apply-new-timing-
...
(def ^:private max-call-depth
  ...
  (e.g. a static-acyclicity gap `ehrt.patient-simulator.gmf/load-closure`'s
...
(defn- run-submodule
  ...
  (when (> (count call-stack) max-call-depth)
    (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: CallSubmodule call depth exceeded max-call-depth -- likely a bug (a static-acyclicity gap gmf/load-closure's own D3 check should have caught)"
  ...
  (loop [callee-ctx (assoc ctx :current :initial :call-stack call-stack) n 0]
    (if (and horizon-end-t (>= (:t callee-ctx) horizon-end-t))
      ...
      (do
        (when (>= n max-steps)
          (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: run-submodule exceeded max-steps -- likely a module authoring bug (a zero-time-advance transition cycle)"
        ...
        (let [outcome (step callee-module rng callee-ctx modules tables horizon-end-t)
          ...
          (cond
            ...
            (:blocked? outcome)
            (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: a called submodule blocked on a Guard -- no resume-across-a-call mechanism this session (Wave B's own disclosed scope limitation, ADR-0027)"
...
(defn- call-submodule-step
  ...
  (the resolved closure, `ehrt.patient-simulator.gmf/load-closure`'s own
  ...
  (let [callee-id (:submodule state)
    ...
    (when (nil? callee-module)
      (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: CallSubmodule names a call-path missing from the resolved closure -- loader/interpreter mismatch (gmf/load-closure should have caught this at load time)"
...
(defn- wellness-wait-step
  ...
  (assert (nil? (open-encounter-index (:trajectory ctx)))
          "ehrt.patient-simulator.gmf-interpreter: nested :encounter (wellness) -- this project's GMF subset assumes encounters never nest (Wave H's own fold discipline, ADR-0042)")
...
(defn step
  ...
  argument, `modules` (`ehrt.patient-simulator.gmf/load-closure`'s own
  ...
  optional 5th argument, `tables` (`ehrt.patient-simulator.gmf/load-
  ...
  ([module rng ctx modules tables horizon-end-t]
   (let [module-id (:id module)
    ...
    (case (:type state)
      ...
      ;; SetAttribute.process, source-grounded) -- `ehrt.patient-simulator.gmf`'s
      ...
      ;; patient-simulator/vital-signs.edn's own closed vocabulary, the
      ...
      (let [name (:vital-sign state)]
        ...
        (let [v (cond
                  ...
                  (:distribution state) (sample-distribution rng (:distribution state))
                  :else (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: VitalSign state has no exact/range/distribution"
...
(defn walk-module
  ...
  ([module rng ctx modules tables]
   (loop [ctx (update ctx :root #(or % (:id module))) n 0]
     (when (>= n max-steps)
       (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: walk-module exceeded max-steps -- likely a module authoring bug (a zero-time-advance transition cycle)"
...
(defn run-module
  ...
  GMF coverage Wave D stage D3 (2026-08-02, ADR-0029, D3a, H2): the
  optional 9th argument, `tables` (`ehrt.patient-simulator.gmf/load-
  closure`'s own `:tables` return shape), threads straight through to
  ...
  ([module rng persona registration-t horizon-end-t modules initial-attributes tables history?]
   (loop [ctx (-> (initial-context persona) (assoc :root (:id module)) (update :attributes into initial-attributes)) n 0]
     (when (>= n max-steps)
       (throw (ex-info "ehrt.patient-simulator.gmf-interpreter: run-module exceeded max-steps -- likely a module authoring bug (a zero-time-advance transition cycle)"
                        {:module (:id module) :current (:current ctx)})))
