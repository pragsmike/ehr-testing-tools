...
(def ^:private census-site-3
  "Section 2, column `site 3 -- reinstated-state`: ten PRESENT
...
(deftest projections-match-the-census-matrix
  ...
  (testing "each site's projection is its own matrix column"
    ...
    (is (= census-site-3 fold/reinstated-projection)
        "site 3 -- reinstated-state's fallback, ten of thirteen"))
  ...
  (testing "the matrix's own arithmetic: 34 present cells of 39, and the
            5 omitted ones are what stage 2 enables pair by pair"
    (is (= 34 (+ (count fold/run-loop-projection)
