...
(def ^:private census-site-2
  "Section 2, column `site 2 -- replay`: six PRESENT cells -- three
...
(deftest projections-match-the-census-matrix
  ...
  (testing "the matrix's own arithmetic: 22 present cells of 39, and the
            17 omitted ones are what stage 2 enables pair by pair"
    (is (= 22 (+ (count fold/run-loop-projection)
                 (count fold/replay-projection)
                 (count fold/reinstated-projection))))
    (is (= 17 (- (* 3 (count fold/full-algebra))
