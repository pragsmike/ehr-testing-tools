...
(def ^:private census-site-3
  "Section 2, column `site 3 -- reinstated-state`: nine PRESENT
...
(deftest projections-match-the-census-matrix
  ...
  (testing "each site's projection is its own matrix column"
    ...
    (is (= census-site-3 fold/reinstated-projection)
        "site 3 -- reinstated-state's fallback, nine of thirteen"))
  ...
  (testing "the matrix's own arithmetic: 33 present cells of 39, and the
            6 omitted ones are what stage 2 enables pair by pair"
    (is (= 33 (+ (count fold/run-loop-projection)
                 (count fold/replay-projection)
                 (count fold/reinstated-projection))))
    (is (= 6 (- (* 3 (count fold/full-algebra))
