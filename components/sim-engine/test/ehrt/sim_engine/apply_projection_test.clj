...
(def ^:private census-site-2
  "Section 2, column `site 2 -- replay`: ten PRESENT cells -- three
...
(deftest projections-match-the-census-matrix
  ...
  (testing "the matrix's own arithmetic: 26 present cells of 39, and the
            13 omitted ones are what stage 2 enables pair by pair"
    (is (= 26 (+ (count fold/run-loop-projection)
                 (count fold/replay-projection)
                 (count fold/reinstated-projection))))
    (is (= 13 (- (* 3 (count fold/full-algebra))
