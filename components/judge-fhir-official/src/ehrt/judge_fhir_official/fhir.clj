...
(defn- resolve-igs
  ...
  :profile), returning kernel/ok [path ...] or the first rejection/
  ...
  (reduce (fn [acc {:keys [name version]}]
            (if-not (kernel/ok? acc)
              ...
              (let [r (resolve-artifact artifacts name version)]
                (if-not (kernel/ok? r)
                  (reduced r)
                  (kernel/ok (conj (:payload acc) (:path (:payload r))))))))
          (kernel/ok [])
          ig-refs))
  ...
  (reduce (fn [acc {:keys [name version]}]
            (if-not (kernel/ok? acc)
              ...
              (let [r (resolve-artifact artifacts name version)]
                (if-not (kernel/ok? r)
                  (reduced r)
                  (kernel/ok (conj (:payload acc) (:artifact (:payload r))))))))
          (kernel/ok [])
          ig-refs))
...
(defn- verdict-cache-lookup
  ...
  [path {:keys [artifacts ig-refs fhir-version verdict-cache-dir resolve-artifact]
         :or {ig-refs [] fhir-version default-fhir-version resolve-artifact kernel/resolve-artifact}}]
...
(defn execute
  ...
  Returns kernel/ok {:invocation :outcome-path :raw-outcome :engine
  ...
  (let [validator-resolve (resolve-artifact artifacts engine-name "6.9.12")]
    (if-not (kernel/ok? validator-resolve)
      validator-resolve
            ...
            (if-not (kernel/ok? ig-resolve)
              ...
              (let [resolved-java-bin (:path (:payload java-bin-result))
                ...
                (if-not (kernel/ok? invocation-result)
                  invocation-result
                  (kernel/ok {:invocation (:payload invocation-result)
...
(defn execute-batch
  ...
  :input-paths (a seq, in argv order). Returns kernel/ok {:invocation
...
(defn gate-batch
  ...
  Returns kernel/ok {:results [{:verdict :findings [:cause] :path} ...]}
