...
| `clinical-trajectory` | de-facto built as data, no formal Malli type yet — dated clinical events, each citing `{module, state}`, codes as `{:system :code :display}` (`ehrt.patient-simulator.gmf-interpreter/run-module`'s own `:trajectory` output) | de-facto built |
| `compiled-pathway`, `authored-pathway`, `pathway-ir`, `operational-pathway` | Malli, `ehrt.sim.pathway` — the union binds to `[:or …]` of its members per the notation; `operational-pathway`'s type IS the IR type (the endomorphism law). `compiled-pathway` is real as of M5b (`ehrt.patient-simulator.compile-trajectory`, six new step types plus the `:citation`/`:conditions` provenance fields) | v1 built |
...
| `gmf-module-set` | 3 — hashed repo-authored/derived config | **RESOLVED, sim/ADR-0013** (author-ratified 2026-07-27): a small, curated subset vendored into `resources/modules/`, hashed and provenance-tracked per-module in a `resources/modules/NOTICE` file (the same role `resources/demographics/NOTICE` plays), not a lockfile (target 1) — sim/ADR-0003's own trigger, decided once `components/patient-simulator/docs/gmf-interpreter.md`'s candidate-module survey gave the question something concrete to be decided against. Explicit revisit trigger: a lockfile, if the vendored set ever grows past roughly ten modules |
...
1. **`gmf-module-set`'s catalytic target** — vendor (3) vs lockfile
   ...
   **RESOLVED (sim/ADR-0013), the same "leave the original entry standing,
   ...
   citation. `components/patient-simulator/docs/gmf-interpreter.md`'s own candidate-module survey
