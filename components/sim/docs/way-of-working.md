...
`test/ehrt/sim/fixtures/fixture-clinic.json` (ADR-0013 point
...
- **Result-not-throw and category honesty** — ADR-0001 point 4
  ...
  authoring error wearing a 'bug in us' category." `ehrt.sim.run/
  ...
  `ehrt.sim.facility/allocate` no longer throws on capacity
...
- **The CLI-surface rule** ("demos and verification commands run
  ...
  `ehrt.sim.engine/run` from a direct API caller
  (`engine-test`) but never from `ehrt.sim.run/run-command` —
...
- **Fixture-as-tripwire and ADR-0009's perturbation-is-policy
  ...
  (`test/ehrt/sim/fixtures/pinned_seed_42_patients_5.edn`) is
...
- **Fix-forward with disclosure** — when a real gap in ground truth
  ...
  `ehrt.sim.engine/evolve`'s `:cancel-discharge` method now
...
- **Mutual accommodation between sibling validators** — two codebases
  ...
  agree on the same mistake (`ehrt.sim.v2-replay`, described in
