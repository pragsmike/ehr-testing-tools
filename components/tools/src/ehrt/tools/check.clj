(ns ehrt.tools.check
  ...
  ehrt.tools.check.schemas' registry). No arbitrary predicate
  ...
  canonicalizer list `cs` (ehrt.tools.canonical) -- equivalence
  ...
  ehrt.tools.diff (promoted from corpus.mutate's own diffing,
  now a second real consumer); unpaired files are reported as
  missing/extra findings.

  Report: reuses ehrt.tools.judge.report's aggregation verbatim
...
(defn assertion-findings
  ...
  or a seq of findings (ehrt.tools.judge.finding-shaped) when it
