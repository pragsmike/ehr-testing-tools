...
(defn render
  ...
  (let [kinds (branches schema)]
    (str "<!-- Generated by `make formats-event-log` (on `make docsgen`) from BOTH "
         "components/sim-engine/resources/sim-engine/event-schema.edn "
         "(the vocabulary and shapes) and "
         "components/sim-engine/resources/sim-engine/event-examples.edn "
         "(every rendered example below). Either one moves this block. "
