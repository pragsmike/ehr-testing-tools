...
(defn render-use-cases-index-md
  "Assembles docs/use-cases.md's full content -- now a generated
  index, one line per case, over case data alone (no mermaid needed:
  the index links to each case's own page rather than rendering its
  diagram inline)."
  [{:keys [cases]}]
