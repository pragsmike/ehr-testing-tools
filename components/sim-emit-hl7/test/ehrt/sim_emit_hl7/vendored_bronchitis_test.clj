(ns ehrt.sim-emit-hl7.vendored-bronchitis-test
  "Vendoring batch 1 (2026-08-07, ADR-0070, AR-VB1-2/3): the full
  compile-trajectory/engine/emit round trip for `bronchitis.json` -- a
...
(def ^:private bronchitis-json (slurp (io/resource "sim/modules/bronchitis.json")))

(def ^:private loaded-closure (gmf/load-closure "bronchitis" bronchitis-json (constantly nil)))
(def ^:private bronchitis-closure (:payload loaded-closure))

(def ^:private run-config
  {:seed 20260802 :patients 300 :pathway {:name "module-only" :steps []}
   :modules [bronchitis-closure] :module-assignment [{:module-id "bronchitis" :weight 1}]
   :module-horizon-days 36500})

(deftest engine-run-completes-real-bronchitis-closure-content
