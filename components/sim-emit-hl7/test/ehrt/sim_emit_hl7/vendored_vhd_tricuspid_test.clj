(ns ehrt.sim-emit-hl7.vendored-vhd-tricuspid-test
  "Vendoring batch 3 (2026-08-07, ADR-0072, AR-VB3-1/2): the full
  compile-trajectory/engine/emit round trip for `vhd_tricuspid.json` --
  a closure of root plus the called `heart/vhd_risks.json` submodule
  (shared with `vhd_pulmonic`, landed there, reused here byte-identical
  at this pin, no new NOTICE row for it) plus two lookup tables
  (`vhd_tr.csv`/`vhd_ts.csv`, `lookup_table_transition`-driven
  severity/prognosis content) -- the census substance artifact's own
  `:closure-file-count 2` UNDERCOUNTS this closure the same way
  ...
  `:event-counts [3 3 3]`, invariant across all three census seeds.
...
(def ^:private vhd-tricuspid-json (slurp (io/resource "sim/modules/vhd_tricuspid.json")))
...
(def ^:private loaded-closure
  (gmf/load-closure "vhd-tricuspid" vhd-tricuspid-json resolve-call-path resolve-table-name))
(def ^:private vhd-tricuspid-closure (:payload loaded-closure))

(def ^:private run-config
  {:seed 20260802 :patients 300 :pathway {:name "module-only" :steps []}
   :modules [vhd-tricuspid-closure] :module-assignment [{:module-id "vhd-tricuspid" :weight 1}]
   :module-horizon-days 36500})

(deftest engine-run-completes-real-vhd-tricuspid-closure-content
