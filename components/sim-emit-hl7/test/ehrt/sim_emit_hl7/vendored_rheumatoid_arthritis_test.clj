(ns ehrt.sim-emit-hl7.vendored-rheumatoid-arthritis-test
  "Vendoring batch 2 (2026-08-07, ADR-0071, AR-VB2-1/2): the full
  compile-trajectory/engine/emit round trip for `rheumatoid_arthritis.json`
  -- a single-file closure (no CallSubmodule, no lookup tables, the
  census substance artifact's own `:closure-file-count 1`). Population/
  horizon sizing follows this repo's own 'measure, don't guess'
  discipline (`ehrt.sim-emit-hl7.vendored-sepsis-test`'s own docstring),
  confirmed empirically this session (seed 20260802, 300 patients, a
  100-year horizon)."
...
(def ^:private rheumatoid-arthritis-json (slurp (io/resource "sim/modules/rheumatoid_arthritis.json")))

(def ^:private loaded-closure
  (gmf/load-closure "rheumatoid-arthritis" rheumatoid-arthritis-json (constantly nil)))
(def ^:private rheumatoid-arthritis-closure (:payload loaded-closure))

(def ^:private run-config
  {:seed 20260802 :patients 300 :pathway {:name "module-only" :steps []}
   :modules [rheumatoid-arthritis-closure] :module-assignment [{:module-id "rheumatoid-arthritis" :weight 1}]
   :module-horizon-days 36500})

(deftest engine-run-completes-real-rheumatoid-arthritis-closure-content
