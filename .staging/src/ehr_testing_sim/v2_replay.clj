(ns ehr-testing-sim.v2-replay
  ...
  ehr-testing-sim.emit-hl7 renders through, plus that namespace's own
  ...
  this accumulator's own state must agree with ehr-testing-sim.engine's
  ...
  (every message carries full PID enrichment, ehr-testing-sim.emit-hl7's
  ...
  ehr-testing-sim.v2-replay-test's own property runs churn EXCLUDING
...
(defn- hl7-instant->seconds
  "The inverse of ehr-testing-sim.emit-hl7/hl7-timestamp: an HL7
...
(defn- hl7-date->iso
  ...
  ehr-testing-sim.emit-hl7/pid-segment's own dash-stripping."
  [raw]
...
(defn- parse-persona
  "Every message carries this uniformly (ehr-testing-sim.emit-hl7/
...
(defn- parse-payer
  "IN1 rides admission alone (ehr-testing-sim.emit-hl7/in1-segment's own
...
(defn- seg-field-components
  ...
  one already-fetched segment (ehr-testing-sim.v2-replay's own callers
...
(defn- parse-obx
  ...
  reference range, OBX-8 abnormal flag -- ehr-testing-sim.emit-hl7/
...
(defn- evolve-entry
  ...
  ehr-testing-sim.engine/evolve's own shape one layer up the wire --
...
;; carries (M6 Task 2's own deliverable, sibling of ehr-testing-sim.
;; site-profile's masking function) ------------------------------------------

(defn project-to-wire-visible-fields
  "Projects a folded PatientState (ehr-testing-sim.engine's own
  ...
  shapes) down to exactly what ehr-testing-sim.emit-hl7's own rendering
  ...
  - :conditions, :medication-orders -- ehr-testing-sim.emit-hl7 renders
  ...
  - :persona's own :ssn/:age -- PID never carries either
    (ehr-testing-sim.emit-hl7/pid-segment's own fixed field list).
  ...
  - Every :observations entry's :codes is narrowed to its FIRST concept
    ...
    ehr-testing-sim.emit-hl7 finding, not introduced by this property:
    ...
    boundary (a real future ehr-testing-sim.emit-hl7 fix, out of this
  ...
  - Events with no message-type-registry entry at all (:step-rejected,
    ...
    ehr-testing-sim.emit-hl7/message-type-registry-covered events,
