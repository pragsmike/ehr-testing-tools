(ns ehr-testing-sim.identifiers
  ...
  (ehr-testing-sim.emit-state's HTEST/run-tag law).
  ...
  A PROJECTION over `ehr-testing-sim.engine/run`'s own output, reusing
  this project's existing single-sourced id-derivation functions
  (`ehr-testing-sim.emit-hl7/control-id-for`, `ehr-testing-sim.emit-
  state/bundle-run`'s own resource ids) rather than re-deriving any of
  them independently, and reusing `ehr-testing-sim.run`'s own config-
  ...
  (:require [ehr-testing-sim.emit-hl7 :as emit-hl7]
...
(defn- final-world
  ...
  the same fold `ehr-testing-sim.emit-state/snapshot-at` computes for
...
(defn identifiers-command
  "opts: the SAME config surface `ehr-testing-sim.run/run-command`
  accepts (:seed required, :patients, :config, plus every
  `ehr-testing-sim.engine/config-keys` entry) -- this is a projection
  ...
  (`ehr-testing-sim.emit-state/bundle-run`, the SAME resource-id
  ...
  one real builder, and returns Result-wrapped:
    ...
    {:run-id             this run's own seed, as a string -- the SAME
                          value ehr-testing-sim.emit-state's meta.tag
  ...
  test/ehr_testing_sim/identifiers_test.clj)."
