(ns ehr-testing-sim.gmf-interpreter
  ...
  two-phase run (Task 3, `ehr-testing-sim.gmf-interpreter.horizon` or
  ...
  (:require [ehr-testing-sim.gmf :as gmf])
...
;; --- RNG primitives (fixed-consumption law, per ehr-testing-sim.engine/
;; ehr-testing-sim.persona's own precedent) ----------------------------------
...
(defn- weighted-pick-transition
  ...
  ehr-testing-sim.engine/assign-pathway and
  ehr-testing-sim.order-profiles/sample-analyte-value already establish."
  [^Random rng entries]
...
(defn step
  ...
  the same decide/evolve-style separation ehr-testing-sim.engine already
...
(defn walk-module
  ...
  (loop [ctx ctx n 0]
    (when (>= n max-steps)
      (throw (ex-info "ehr-testing-sim.gmf-interpreter: walk-module exceeded max-steps -- likely a module authoring bug (a zero-time-advance transition cycle)"
...
(defn run-module
  ...
  ([module rng persona registration-t horizon-end-t]
   (loop [ctx (initial-context persona) n 0]
     (when (>= n max-steps)
       (throw (ex-info "ehr-testing-sim.gmf-interpreter: run-module exceeded max-steps -- likely a module authoring bug (a zero-time-advance transition cycle)"
                        {:module (:id module) :current (:current ctx)})))
