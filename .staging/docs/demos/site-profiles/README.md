...
passthrough (`ehr-testing-sim.run/run-command`'s docstring), the same
...
`test/ehr_testing_sim/emit_hl7_test.clj`, for exactly which two are
...
(`ehr-testing-sim.site-profile/code-for`). **ZPI** is new: a Z-segment
