...
| `Delay` | v1, consumed internally | none — advances simulated time only. Compiles to the same `:advance N` shape every other step type's `decide` already returns (`ehr-testing-sim.engine`); the interpreter samples the delay's own authored range from the run's single RNG, same determinism law every other stochastic draw already follows |
...
| `Procedure` | v1, trajectory event | compiles to a new `:procedure` IR step (does not exist in `ehr-testing-sim.pathway` today — M5b build scope) |
...
their operational encounter horizon begins (`ehr-testing-sim.engine`'s
