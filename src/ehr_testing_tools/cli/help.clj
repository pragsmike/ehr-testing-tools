   ...
   [{:group "artifact"
    ...
    {:group "corpus"
     ...
     [{:verb "generate" :doc "Generate a deterministic synthetic Synthea corpus. Zero-flag defaults (D9, ADR-0019) make the bare command a byte-reproducible run -- re-running it into the same (derived) --out-dir without clearing it first is rejected (:out-dir-exists), not silently overwritten."
      ...
      {:verb "intake" :doc "Catalog a foreign (not generated by this repo) corpus batch -- or, given a generator URL (sim:?seed=42, synthea:?seed=1&population=5) in place of PATH, generate the corpus first and then catalog it, in one command (SS-2)."
       :flags [{:flag "--path" :doc "alternative to the positional PATH"}
       ...
       :positional "PATH"
       :positional-doc "a directory of files to catalog, OR a generator URL (sim:/synthea:) naming a corpus to generate then catalog, given as a trailing positional argument, not --path -- an explicit --path is never overridden by it"}
