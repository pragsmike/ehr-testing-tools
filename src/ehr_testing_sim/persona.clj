(ns ehr-testing-sim.persona
  ...
  law `ehr-testing-sim.engine/assign-pathway` and `ehr-testing-sim.churn`
...
(def given-names-by-sex-and-decade
  ...
  (edn/read-string (slurp (io/resource "demographics/given-names.edn"))))

(def surnames
  "Loaded once: resources/demographics/surnames.edn -- a flat weighted
  pool, [{:name :weight} ...]."
  (edn/read-string (slurp (io/resource "demographics/surnames.edn"))))

(def places
  "Loaded once: resources/demographics/places.edn -- a flat weighted
  pool of full address rows, [{:street :city :state :zip :weight} ...]."
  (edn/read-string (slurp (io/resource "demographics/places.edn"))))
...
(defn- weighted-pick
  ...
  weight bucketing, the same shape ehr-testing-sim.engine's own private
...
(def ^:private reference-birth-year
  ...
  2024 matches ehr-testing-sim.emit-hl7/default-reference-date's own
...
(defn persona
  ...
  ehr-testing-sim.engine/assign-pathway and ehr-testing-sim.churn/
