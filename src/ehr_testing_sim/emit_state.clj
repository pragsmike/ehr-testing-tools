(ns ehr-testing-sim.emit-state
  ...
  1. Snapshot-at-instant: `snapshot-at` is a pure function of REPLAY
     RECORDS (ehr-testing-sim.engine/replay's own output -- the fold,
     ...
     fold this law is about, the same one ehr-testing-sim.check already
  ...
  3. Cross-emitter id sub-law (the GLOBAL emitter-coherence law, docs/
     ...
     SAME identifiers ehr-testing-sim.emit-hl7 renders -- Patient.id is
     the patient-id ehr-testing-sim.engine/patient-id-for assigns
     (never a fresh UUID), Patient.identifier carries the active-mrn
     ehr-testing-sim.emit-hl7 renders as PID-3, and every other
  ...
  4. Minimal-but-valid, no invented fields: the rendered resource set is
     ...
     Coverage, each populated ONLY from fields ehr-testing-sim.engine's
     own fold already carries (this namespace's own header comment on
     ehr-testing-sim.engine/PatientState records what had to LAND in
...
(defn snapshot-at
  ...
  given `replay-records` (ehr-testing-sim.engine/replay's own output).
...
(defn- iso-timestamp
  "The same pinned-clock arithmetic ehr-testing-sim.emit-hl7/hl7-timestamp
...
(defn- patient-resource
  ...
  (when persona
    {:resourceType "Patient"
     :id patient-id
     :identifier [{:system "urn:ehr-testing-sim:mrn" :value active-mrn}]
