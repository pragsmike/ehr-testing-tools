(ns ehr-testing-sim.site-profile
  ...
  emitter argument: all three bind at ehr-testing-sim.emit-hl7's render
  ...
  (nil) -- ehr-testing-sim.emit-hl7 renders identically whether no
  ...
  member of ehr-testing-sim.engine/config-keys at all -- structurally
  ...
  time (ehr-testing-sim.facility/surge-slot-ids, a decision that
  ...
  applies to `:facility` BEFORE ehr-testing-sim.engine/run, not
  something ehr-testing-sim.emit-hl7 ever reads. Calling it a
...
(defn apply-naming
  ...
  ehr-testing-sim.engine/run, if at all: never automatically wired into
...
(def ZFieldBinding
  ...
  ehr-testing-sim.emit-hl7 -- state/persona/event paths, e.g. [:persona
