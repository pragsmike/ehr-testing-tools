...
close (`notes/adr/0084-fidelity-arc-close.md`, AR-FC-5) — this citation
deliberately STAYS at ADR-0084 through this commit (the conviction
arc's own Step 1, AR-CB-1): the staleness tripwire
(`state_staleness_tripwire_test.clj`, ADR-0079's own gate) checks
COMMITTED state, and `notes/adr/0089-conviction-arc-close.md` (this
arc's own closing ADR) does not exist on disk until Step 2 lands it —
that commit moves this citation to ADR-0089 in the SAME commit that
creates the file, closing the sequencing rather than evading it, the
exact ADR-0080/0084 Step 2/Step 3 precedent, re-exercised across this
close's own two-session split. Every section below is regenerated
fresh THIS session regardless — only this header sentence deliberately
holds its prior value one commit longer. Landed against tip `a9c3abf`
