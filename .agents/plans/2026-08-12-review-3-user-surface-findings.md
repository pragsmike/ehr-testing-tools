...
**Addendum, 2026-08-12 (ADR-0118): R3-B3-1's own no-witnessed-invocation gap.**
Fixing R3-B3-1 required a real, verbatim invocation for each group,
drawn from README.md's Quickstart, `docs/use-cases/*.md`, or a demo
README (B2's own sourcing rule, [C, approved by dispatch of the
driving prompt]) -- never a composed one. A repo-wide grep of all
three source classes for `bin/ehrt version` and `bin/ehrt doctor`
found zero instances of either, in any file, this session. Per the
rule's own "if a group has no witnessed invocation anywhere, render no
Example for it" clause, `version` and `doctor` render no `Example:`
line -- not a defect, a disclosed, honest gap, recorded here rather
than silently left unexplained by the fix. Either group would need a
real worked invocation to land in one of the three witnessed source
classes (most naturally `docs/use-cases/*.md`, since neither is a
Quickstart or demo step) before this addendum can close.
...
with a cross-reference, "(see ...)", and excluded from that battery's
own disposition counts to avoid double-tallying -- it is still one row
in the table, just not one vote in the sum:
...
**Correction, 2026-08-12 (ADR-0115):** this table's own note above
originally claimed a cross-referencing row is "marked '(x-ref)'" --
the actual marker every such row carries in its own disposition cell
is a cross-reference citation, "(see R3-Bx-y, not double-counted)"
style text, never the literal token "(x-ref)"; the note's wording is
corrected in place above. Counts unaffected -- independently recounted
by the design channel, 2026-08-12. This table is the review's own
review-time snapshot: the `ruling-needed` column and every row's
disposition entry below reflect dispositions AS OF the review's own
landing (ADR-0114); three of those dispositions have since moved
(R3-B1-1/4 to `fix-session-candidate`, R3-B1-7 to `closed-by-ruling`,
ADR-0115) -- the rows themselves carry the current state, this table
does not, so a future reader comparing the two isn't looking at a
contradiction.
