...
- **Verify after each commit**: full test suite + both integration suites + `make pipeline` regenerates `pipeline.md` cleanly + tier-1 lint passes.
  **2026-07-27 dated note (ADR-0016):** this line was sized for Phase 1's own renaming sweep, executed and Done — it is not itself in error, and is left as written above for the historical record. It no longer generalizes as a standing per-commit rule: `notes/ADRs.md` ADR-0016 and `AGENTS.md`'s "Verification tiers" section now govern what a commit owes going forward — T0 (`make test` + both lints + `quickstart-fresh`) per commit, T1 (`make integration-smoke`) at session boundaries, T2 (both integration suites) only when the change set touches the trigger list ADR-0016 states, plus nightly as the backstop.
