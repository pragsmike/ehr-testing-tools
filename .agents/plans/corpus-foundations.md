...
| P5 | EXP-SBOM mechanical half (dependency/license SBOM against the MIT target, per ADR-0007), EXP-C5 (FHIR validator offline behavior), EXP-D3 (CDC wrapper offline build) | Not started | — |
...
| First release | Gates capability landed (`gate.fhir`/`gate.v2`); the resource-equation notation trial (pattern nursery #13) concluded one way or the other; coverage-threshold gating landed. Version tag, published coordinates (Clojars vs. Maven Central, `docs/positioning.md` open decision), guide-repo cross-references begin. | Not started — the milestone after publication (ADR-0008) | — |
