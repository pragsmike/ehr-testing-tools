...
The tabulator is `bin/event-census` (Appendix A) — a plain Clojure
script over the emitted EDN with no repo dependency, so every number
below can be re-derived, including against a log this checkout's own
schema would reject.
...
_(Tables generated by `bin/event-census` over all eleven corpora.)_
...
**Promoted to [`bin/event-census`](../../bin/event-census)** by author
ruling (2026-08-16, licensed fence widening) — the source now lives at
`bin/event-census-src/ehrt/event_census/census.clj`, so this census is
re-derivable rather than ageing inside a plan file. Every table above
was re-generated by the promoted script and diffed against what this
file already carried: **identical**.

```bash
bin/event-census out/scenarios/ed-tuesday/events.edn \
                 out/scenarios/clinic-decade/events.edn \
                 out/census/*.edn out/census/*.out
```

It reads the bare ground-truth vector, `corpus generate sim`'s
`events.edn`, or the full `sim run` envelope, and **skips** a kernel
error envelope with a note rather than counting it as an empty run.

Like `bin/fence-census`, it depends on nothing it audits: its
population is the corpora, and it deliberately does not load the
`Event` schema — an instrument that read the contract to decide what
to count could only ever confirm the contract agrees with itself.
`ehrt.sim-engine.event-schema-test` is the gate; this is the survey.

The script as it stood when this census ran, kept for the record:
