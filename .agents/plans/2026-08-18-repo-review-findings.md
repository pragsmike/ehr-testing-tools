...
**ARC CLOSED 2026-08-20, `notes/adr/0159-review-4-arc-close.md`.** All
**38** rows that moved are marked by their own fix session's dated
append; the **34** residue rows carry a dated `CONFIRMED` / `CARRIED` /
`SUPERSEDED` append from the close. Every fix-session-candidate (27) and
every ruling-needed row (10) moved. **Read ADR-0159 before re-deriving
this file's arithmetic** -- it verifies all 38 citations against the
ADRs that claim them, partitions the residue, and lists three figures in
this register and its plan that do not re-derive.
...
| D2-5 | Is there a gate against the exit-code-masking class in its NEW shape? (review 3's H-2/H-3 watch item, which said to watch for a new way, not the old defect) | See the L-2 section: the taught law forbids a pipe or `tail` on the GATE command; ADR-0152's mask was in the WRAPPER's last command, which the law does not mention. `git grep -niE 'MAKE_EXIT\|masked' -- '*test*.clj'` → no gate of any kind. | **The watch item fired.** Recorded here as a guard-coverage fact; counted and evidenced in L-2 (L2-1), not tallied twice. | See L2-1. | **cross-reference to L2-1, not separately tallied**; **SUPERSEDED ADR-0159 (2026-08-20)** -- closed by its referent: L2-1 is FIXED ADR-0155. |
...
## Close note, 2026-08-20 (ADR-0159) — the final disposition tally

Appended by the arc close. **No review-day row or score above is
edited**; the dispositions are dated appends in the rows' own
disposition cells, in the shape the fix sessions used.

Re-derived mechanically from the live rows (72, matching this file's own
count), never from the summary line:
...
Moved, by citing ADR: **ADR-0155** 12, **ADR-0156** 11, **ADR-0157** 2,
**ADR-0158** 13. Each matches `roadmap.md`'s own per-session tally
exactly — ledger delta **zero** in every session.

Residue, by bucket: **24** close-as-fine confirmed (six re-derived at
the close: D1-3, D1-4, D2-3, D3-3, D5-4, L2-11), **8** intake carried by
this register and review 5's watch-list, **1** intake also rowed
(D7-2 → `roadmap.md#attic-rotation-law`), **1** superseded by its
referent (D2-5 → L2-1, FIXED ADR-0155).

**Three figures in this document and its plan do not re-derive**, listed
in ADR-0159 under `R-RP` and deliberately not rewritten here: the skills
mirror is **59** files, not the 60 D2-3 and D5-4 and the plan's Part 3
state; `state-derived.md` is counted by **zero** reading sets, not the
four the plan's Session H watch line states; and ADR-0158 records
`:onboarding` 1482 where the generated register it commits says 1484.
