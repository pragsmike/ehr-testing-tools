...
- OPEN **[event-mutation-catalog-gate]** PRIORITY 6 -- the two live follow-ons
  ...
  spurious occupancy ones. Its cost -- twelve operators over two real runs --
...
- CLOSED 2026-09-04 963902d **[dense-scale-profile]** -- CREATED AND CLOSED IN ONE ENTRY, and DISCLOSED AS CREATED: no such row existed when the 2026-09-04 (a) session went looking for it, and it is written here only because the work it would have named is now done. The row `docs/consuming-ground-truth.md#scale` needed was "give the three published Scale cells a runnable artefact and re-measure them". Session (a) committed the artefact -- `demos/scenarios/dense-7500/`, three configs whose relationship is mechanically checked, a README, an exerciser and a register row -- and then STOPPED: `config-bare.edn` at 7,500 arrivals died on `:capacity-exhausted` 2,963 arrivals in, so the third cell had no re-measurement and the table was left whole rather than half-rewritten. THE DESIGN CHANNEL RULED (b), correct the skeleton, and LITTLE'S LAW NAMED THE DEFECT EXACTLY rather than merely bounding it: `:arrival-gap 2` gives lambda = 1.00 arrivals/minute, the module cohort takes every eighth ordinal and the rest splits 45/35/20, so `dense-inpatient` arrives at 0.394/minute and its 1,020-minute Medicine A dwell is a steady-state census of **401.6 against a 240-bed ward** -- 167% of it before a single overflow. That also explains why the refusal payload named SURGERY and not Medicine A: `facility/allocate`'s ladder spills into other inpatient wards' LICENSED beds and never their surge, so a Medicine A overrun eats Medicine B's and Surgery's licensed beds until Surgery's own traffic has nowhere to go. Five delays shortened (30-90 / 45-120 / 120-480 / 240-720 / 120-360 to 10-30 / 15-45 / 30-120 / 60-180 / 30-90), ward sizes untouched per R-b; Medicine A now sits at 100.4, Emergency at 106.0, total inpatient census 273.7 against 680 reachable inpatient beds. ALL EIGHT TIMED RUNS EXIT 0 and both runs of every cell agree to the event: `config.edn` 7,500 = 167,190 ev / 222,748 msg / **1.3323** / 281.46 s / 2,209 MB; `config-nobed.edn` = 125,825 / 164,217 / **1.3051** / 226.25 s / 1,866 MB; `config-bare.edn` = 100,884 / 65,239 / **0.6467** / 144.29 s / 1,817 MB; `config.edn` 750 = 33,303 / 40,281 / **1.2095** / 53.84 s / 1,196 MB. The bare cell's 0.6467 lands within 0.6% of the 2026-08-29 programme's own 0.643 on the configuration that no longer exists -- corroboration arriving from a direction the session did not arrange. Two derived claims moved and ONE DIED: the bed cycle is 41,365 events (24.7%) and 58,531 messages, and it is message-RICHER than the log it rides on (1.4150 messages per added event), so the previous generation's "msg/event unmoved to four places" was a property of the old dwells and is replaced rather than dropped. THE PREVIOUS RECORD'S MECHANISM WAS WRONG AND IS CORRECTED IN THE DOCS: `:persons` does not keep the census inside capacity -- `owner-ordinal` resolves a repeat arrival to the first arrival's ordinal, merging RECORDS, and both arrivals still hold a bed. `:scheduling` is the throttle: `decide :appointment` sets `scheduled-t` to the booking instant plus `:lead-seconds`, and at this file's 0.70 / [3 21] / 0.08 / 0.15 that defers 54% of a 5.2-day arrival stream across three weeks and drops 16% of it. All five referential carrier columns SURVIVE the correction (A 2,230, B1 10,253, B2 4,884, C 3,486, D 943), the two shipped ones cross-checking to the site. Full `make test` green at 4,815 tests / 25,700 assertions. Records: `.agents/session-records/2026-09-04-dense-7500-scale-cell.md` (the stop) and `-b.md` (the correction).
- CLOSED 2026-09-03 ADR-0177 **[cancel-transfer-reinstates-a-new-subject]** -- DONE, the engine half, and the supersession is now KIND-AWARE rather than one set grown by a member: this row's own proposed fix ("admit `:new` to `statuses-that-supersede-a-reinstatement`") was REFUTED BY TRACE before any code -- plain admission rejects M6 Task 2's cancel-discharge (conjunct 1, `:new` in the widened set; conjunct 2, `(not= :new :discharged)`) -- so R-A1-scope ruled the asymmetry: `:new` supersedes a `:cancel-transfer`, which reinstates ONLY location/home-ward and so re-creates exactly the state `non-admitted-patients-hold-no-bed` convicts, and deliberately NOT a `:cancel-discharge`, whose undo reinstates `:admitted` + bed + `:class` as a coherent whole (M6 Task 2 green throughout). The per-kind table in `log_index.clj` is the whole src diff; `decide.clj` is untouched, both call sites already passed `kind`, branch order unchanged. The TS-5 boundary pin (`...-is-still-applied`, "pinned so that widening it is a decision and not a drift") was witnessed red under the new guard and rewritten as `...-is-rejected` -- the decision its own docstring demanded, its no-discharge world kept as the second witness shape. WITNESSED at the calibration fixture (seed 424242, `--reference-date 2026-08-31 --churn --format ground-truth`): `--patients` 500/1000/1984/2000 ALL EXIT 0 -- the two formerly-failing arrival counts flip green -- and the 500/1000 SHA-256s MATCH the downstream's own published values byte for byte (`434232a9...`, `ddcfc319...`), so NO `:new` reinstatement occurred in those two runs and the fix changes nothing they measured; ed-tuesday exits 0 at B2's own 424,772 ground-truth bytes. Sweep UNSPENT per R-sweep: `bin/regression-oracle c16bb26..e8016e7` IDENTICAL (every root), `bin/ground-truth-bracket` IDENTICAL (38 roots, same 3 keyless skips as TS-5). The `nobed` 10^5 cell is UNREPRODUCIBLE per fence (`dense-7500-nobed.edn` not in the tree): its 2 left-alone cancel-transfers are reasoned to flip under the ADR, not re-measured. SUPERSEDED IN ONE PARTICULAR 2026-09-05 by ADR-0178: those two SHA-256s no longer match, and the sentence stands as the dated measurement it was rather than being rewritten -- the fix removes 7 `:window-close-t nil` pairs from each payload, the downstream's own simulator carries the same defect, and the two outputs now differ from theirs by exactly those removed pairs and nothing else (proven per root). What the sentence was FOR -- that no `:new` reinstatement occurred in those two runs -- is untouched. Record: `.agents/session-records/2026-09-03-a1-new-supersedes-reinstatement.md`.
- CLOSED 2026-09-01 cfea631 **[event-stream-mutation]** -- DONE, and the arc ends with the CATALOG AT TWELVE operators (9 referential across the two populated carrier columns, 3 structural), each carrying its own closed oracle loop. THE LOOP IS THE DELIVERABLE, not the count: `ehrt sim run --format ground-truth | ehrt sim mutate --operator-id ID --seed N | ehrt sim check` is typeable at a shell, and its contract is Q5(a) SET EQUALITY -- the findings `check` reports on the mutant equal the findings the operator DECLARED, exactly, in both directions, so an operator naming a class it does not produce fails the build just as loudly as one producing a class it did not name. Q1(a) settled the injection point against this row's own inference: the stage is POST-RUN and outside `engine/run` entirely, because `fold/apply-events` sees one decide's batch and not the log, so a mutation folded there becomes a wrong WORLD rather than a wrong RECORD and the engine silently repairs some injections. WHAT THE ARC ACTUALLY PROVED is that MEASUREMENT BEAT THE DESIGN DOC: `.agents/plans/2026-09-01-event-mutation-population-ledger.md` measured every candidate operator's site population over real generated logs at each config's own documented invocation, and refuted ADR-0176 THREE TIMES -- section 2(iv)'s declared population is EMPTY; section 2(i)'s matrix arithmetic UNDER-COUNTS (`:order-event-id` has two carriers convicted by different invariants, so 23 cells and not 20); and all THREE structural shapes as 2(iv) words them do not name a stable defect class, so they shipped NARROWED rather than as declared. All three corrections ride in ADR-0176 as dated addenda instead of being quietly absorbed. The ledger also separates THREE gap kinds that a single word would have blurred -- catalog gaps (unconvictable, refused at registration: ZERO), population gaps (convictable in principle, unwitnessable today: 14 cells, each naming the invariant that would convict it, rowed at `roadmap.md#referential-corpus-population`), and shape gaps -- and NOTHING was invented to fill one. Q12(a)'s `ehrt sim mutate --lineage PATH` writes each application's provenance beside the pipe rather than in it. Q13(a) closed the last carry: RNG family tag 6 is now RESERVED for `:mutation` in `streams.clj`, comment not row, after the spine and breadth sessions each carried it unwritten. LIVE FOLLOW-ONS REMAIN and are their own rows: `roadmap.md#event-mutation-catalog-gate` (the catalog-wide gate, and the `:expected-findings` vocabulary check that ruling Q11(a) blocks) and `roadmap.md#referential-corpus-population` (the 14 cells). ADR-0176 with its addenda. Records: `.agents/session-records/2026-09-01-event-stream-mutation-design.md`, `-spine.md`, `-breadth.md`.
- CLOSED 2026-09-01 3ec147f **[sim-check-takes-no-facility-config]** -- DONE, `ehrt sim check --config PATH`, under ruling Q14(a). The row was RIGHT IN EVERY PARTICULAR: the verb had no flags at all, so all four config-needing invariants ran against the shipped defaults whatever produced the log, and `ed-tuesday` -- whose `:facility` raises the Emergency ward from 6 surge slots to 16 -- read as violating `:occupancy-within-capacity` on its OWN clean log, which made a SHIPPED DEMO'S OWN ORACLE PIPE report a false positive. The corpus was sound; the checker was config-starved. THE ROW'S PROPOSED FIX SURVIVED DERIVATION UNCHANGED, which is why this closed in one session: `ehrt.sim-check.interface` needed NO widening -- it already re-exports `check-all` at all four arities -- so Q14(a) was honored through the EXISTING seam and the Q11(a) fence stands untouched. The narrow points were the corpus adapter (calling the 1-arg arity) and the CLI verb (dropping every parsed opt). It threads EXACTLY what the run's own in-process self-check threads, `:facility` and `:warm-up-seconds`, and deliberately NOT `:order-profiles`: threading those in only one of the two would let `check --config X` convict a log `run --config X` had already passed, breaking the pipe in the opposite direction -- so that gap stays symmetric and documented rather than half-closed. The agreement is exact rather than approximate because `sim-engine` echoes `:facility` back VERBATIM (only providers are materialized). TWO THINGS THE WORK FOUND that the row did not predict: `help.clj`'s `:flags` vector is not documentation but the flag WHITELIST `validate-known-flags` reads (AR-U3-2), so `check`'s empty vector rejected `--config` by name at exit 2 while the in-process tests passed -- caught only by running the fixed pipe at a real shell, and now pinned by a `main!`-level test; and `docs/consuming-ground-truth.md` had documented the defect as settled behavior, so the prose was rewritten rather than appended to. Record: `.agents/session-records/2026-09-01-sim-check-facility-config.md`.
- CLOSED 2026-09-01 ec52471 **[engine-namespace-extraction-and-apply-unification]** -- DONE, and the arc ends THIRTY-EIGHT of its thirty-nine (site x accumulator) cells at FULL PRODUCT with the thirty-ninth a MEASURED PERMANENT OMISSION rather than an unfinished pair. TWO PHASES, and the extraction half has its own row: `roadmap.md#engine-emit-namespace-extraction` carries the eighteen landings, both pure facades, the doctrine that phase established, and the ruled repoint pass that cleared this row's three standing backlogs. THE UNIFICATION HALF RAN IN THREE SESSIONS, 2026-09-01. STAGE 1 built the choke point -- `fold/apply-events`, the ONE fold all three apply sites run, each passing the stack it already ran as an explicitly DECLARED SUBSET of `fold/full-algebra`, gated by `ehrt.sim-engine.apply-projection-test`, which TRANSCRIBES the census matrix rather than reading it so the two can disagree loudly (`434e939` census, `7ab7cdb` choke point). STAGE 2 enabled the omitted pairs ONE COMMIT EACH in census order: all NINETEEN predicted INERT confirmed, none refuted, so the cone method itself was never put in question (`f84005a..3abfa44`) -- plus `c4f6ddd`, the DE-ALIAS the census missed, since site 3's projection was site 2's BY IDENTITY and no site-2 pair could be enabled without silently enabling its site-3 twin. THE LANDING SESSION disposed the three predicted OUTPUT-MOVING (`5b13dec`, `c172684`, `ec52471`). WHAT THIS ROW ACTUALLY PROVED IS THE CENSUS'S OWN METHOD, and it proved out by being WRONG TWICE IN A CHECKABLE WAY -- which is the whole value of writing a prediction down before enabling anything. 2 x and 3 x `:encounter-stamp` were predicted OUTPUT-MOVING and are the IDENTITY on every encounter-carrying root the repository has, for TWO INDEPENDENT REASONS either of which would carry it: `encounters/stamp-encounter` guards on `contains?` -- "a key that is there is there" -- and the decoration reads a PRE-BATCH world that is empty at both sites, the whole log arriving as one batch. Both landed under ruling A1(b) WITH `ehrt.sim-engine.apply-restamp-identity-test` co-landed in the same commit, which pins each mechanism BY ITS COUNTERFACTUAL -- a world that WOULD mint a different id, a log with its ids STRIPPED -- because neither mechanism is a property of the pair and a later session could change either without knowing this line rests on it. 2 x `:warm-up-mark` was predicted OUTPUT-MOVING, IS (first divergent entry 0, 2 of 9 entries, byte 425, on a windowed log), and is OMITTED PERMANENTLY under ruling A2(b): `replay` has NO SOURCE for the warm-up window, a declared 0 measurably DESTROYS the log's own authoritative marks, and threading the window would be an API change at 16 live call sites so that `replay` could re-derive a value the log already carries -- the vacuous shape relocated rather than removed. THE ORACLE IS BLIND TO ALL THREE AND SAYS SO ITSELF: `ehrt.oracle.digest`'s own coverage note lists `engine/replay` among the paths NO root reaches, and no gated root runs a warm-up window at all, so an IDENTICAL verdict there is a VACUOUS GATE -- census correction C3's shape arriving from the other direction. Every disposition rests on DIRECT measurement instead; the oracle was run at every span and reported as vacuous rather than as proof. Sites end 13 / 12 / 13, twenty-one of the twenty-two pairs at full product. Census `.agents/plans/apply-unification-census.md` -- six corrections to the extraction census's section 4, its 3d checklist marked pair by pair with landing shas, and 3e the permanent omission. Records: `.agents/session-records/2026-09-01-apply-unification-stage-1.md`, `-stage-2.md`, `-landing.md`. What it did NOT do is `roadmap.md#event-stream-mutation`, whose injection point it built.
