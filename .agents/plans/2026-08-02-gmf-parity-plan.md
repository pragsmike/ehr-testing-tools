...
wave. **Dated note (2026-08-04, ADR-0042): Wave H (pre-roll) landed —
see its own row below. The GMF parity arc is COMPLETE; no further
GMF-coverage wave is scheduled.** Successor
...
see roadmap**) → **G** (wellness — ledger was 19 tagged modules plus
FOUR max-steps loop walk-failures, `med-rec`/`veteran-substance-
abuse-treatment`/`mend-program`/`metabolic-syndrome-care` — **DONE,
ADR-0037, see roadmap: the create-now substitution retires, all four
loop modules resolve fully to `:ok-walked`, `gallstones` reclassifies
`:out-of-scope-by-ruling`**) → **LC** (lookup-table columns generalize,
9 modules — **DONE, ADR-0038, see roadmap**) → **VS** (the vital-sign
channel, minimal baseline set — **DONE, ADR-0039, see roadmap: register
+ `VitalSign` state + `:vital-sign` condition land; `covid19`/
`contraceptives` resolve fully, `congestive-heart-failure` unmasks an
unrelated `SetAttribute` conflict, `wellness-encounters` advances one
state (`Height`→`Weight`), `metabolic-syndrome-care` unchanged/
...
a re-ordering of the table row order above: H was provisionally
second-to-last (…LC→VS→H→I); it now runs absolute last (…LC→VS→I→H).
H's own history-phase design should exercise against the complete
walking catalog, not a partial one — moving it last means every other
frontier this plan names closes first. **Wave E is RE-SCOPED**:
`stroke.json`
...
| **LC** (ADR-0038, ratified order) | Lookup-table columns generalize from a two-column (`age`/`gender`) whitelist to ANY attribute column, module-attribute-first + persona-field resolution + honest absence; persona `:state` field added. | 1 | 9 modules resolve fully to `:ok-walked`, zero surfaced a next blocker |
| **VS** (ADR-0039, ratified order) | The vital-sign channel, minimal baseline set — per-patient register, `VitalSign` state (sample-once), `:vital-sign` condition (register read + honest absence), six authored baseline constants. | 1 | `covid19`/`contraceptives` resolve fully; `congestive-heart-failure` unmasks an unrelated `SetAttribute` conflict; `wellness-encounters` advances one state (`Height`→`Weight`); `metabolic-syndrome-care` unchanged/latent |
| **H** *(re-ordered LAST, ADR-0039 AR-7 — was provisionally second-to-last)* | **Pre-roll** — design session first. Walk modules deterministically from onset to registration; fold pre-window history into initial patient state; emit only in-window events. Open questions: what folds vs. what is discarded; concurrent-comorbidity interaction (ADR-0027 D1 already rules the channel: clinical state, never scratch); composition with churn and the horizon model. **Ruled (2026-08-03, ADR-0031 AR-3): emit-nothing REAFFIRMED — no backloaded-history mode in the sim; the backload need is a named TOOLS-SIDE future (a corpus construction over sim output), revisit trigger: a real consumer for pre-window messages appears.** **Dated pointer (2026-08-03, ADR-0033's own execution note, carried here by ADR-0034 AR-6 for H's own design session to read): UTI's mandatory Encounter (Care Pathways) straddles `engine.clj`'s own fixed registration-t anchor for most seeds — 8 of 10 sampled trip `check/check-all`'s `:clinical-content-only-when-admitted` invariant (opens pre-horizon, folded only into `:pre-horizon-facts`; closes post-horizon as a real, discrete `:outpatient-visit-end` event). The UTI engine round-trip test dodges this empirically (seed 777, chosen because it doesn't trip it) rather than resolving the boundary — that dodge is a standing, disclosed workaround, not a fix; it retires the moment H's own pre-horizon/post-horizon fold boundary design lands and actually resolves straddling encounters, at which point the round-trip test should be revisited too (does it still need a hand-picked seed, or does any seed now work).** **Dated note (2026-08-03, ADR-0037 AR-2(c)/Wave G): `:wellness-wait` during a future history phase must FOLD state without emitting** — the same discipline H's own pre-roll design already owes every other state type (emit-nothing, `notes/ADRs.md` ADR-0031 AR-3), now explicit for the wellness cycle specifically: a `next-wellness-tick` computed and attached DURING history-phase fast-forward is real clinical state (an actual visit happened), but its own trajectory event must NOT ride the ground-truth wire the way a horizon-phase wellness visit does — H's own design session is where this fold rule gets built, not Wave G (which only proves the wait mechanism itself, at the interpreter layer, `registration-t`/`horizon-end-t` already threaded generically). **Dated note (2026-08-04, ADR-0042 —
LANDED, wave CLOSED):** `:history` opt-in config flag; the interpreter
mints a `:phase` mark (`:history`/`:horizon`) via AR-2's own
encounter-anchored inheritance (an event's phase comes from its own
currently-open encounter's opening phase, not its own raw timestamp);
`compile-trajectory`'s new path drops every `:phase :history` event
uniformly, no registration-facts bucketing. The straddle resolves by
design — the UTI round-trip test now runs an ORDINARY seed under
`:history true` (seed 777 retired, ADR-0042 AR-4) — and a second,
narrower straddle class found live (`:medication-end`/`:care-plan-
end`/`:condition-end` firing outside any encounter, referencing a
dropped antecedent) closes via the same "no orphaned reference to
something dropped" principle one `:references` hop further. `history?`
false stays the pre-H legacy path unchanged, byte-identical
(`bin/regression-oracle 537f954 6a587ff`: all 9 pre-existing roots
IDENTICAL); `ear-infections-history-engine`/`urinary-tract-infections-
history-engine` are the first history-mode oracle baselines.
Carry-across (mid-stay-at-window-open emission) is the named future,
roadmap Deferred. **This was the SOLE remaining wave — the GMF parity
arc is COMPLETE.** | 1 design + 1–2 impl | the chronic/lifetime catalog; acute-episode → population-scale simulator |
