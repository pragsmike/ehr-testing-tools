...
| D2-4 | Which derived artifacts have a freshness gate? (population taken from D5-4's tree-first enumeration, not from the make graph) | CI's freshness step diffs exactly five paths: `docs/dev/pipeline.md`, `docs/use-cases.md`, `docs/use-cases/`, `docs/operators.md`, `docs/cli.md`. **Five further derived artifacts exist in the tree with no gate of any kind** (D5-4). | A gate whose population is the make graph cannot see a derived artifact that was never added to the make graph. Structurally identical to D1-2 (a gate scoped to `docs/`) and D1-6 (a tool scoped to one parent commit). | See D5-4's recommendation — registering the derivations is the fix, and it closes this row too. | fix-session-candidate (**same session as D5-4**) |
