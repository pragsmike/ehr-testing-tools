(ns ehrt.tools.smoke-test
  ...
  `projects/conformance/test/ehrt/tools/smoke_test.clj` in the same
  session, along the seam the original docstring already drew (\"FHIR
  half\" / \"sim-harness half\") -- the sim-harness half needs
  `ehrt.tools.sim-harness`, a conformance-project-local test helper also
...
(deftest ^:integration fhir-clean-vs-mutant-pairing-polarity-smoke-test
  (let [gate-opts {:artifacts @lockfile-artifacts :out-dir work-dir}
        clean-result (gate/fhir-gate-file @base-file gate-opts)
        ...
        mutant-result (gate/fhir-gate-file mutant-path gate-opts)]
