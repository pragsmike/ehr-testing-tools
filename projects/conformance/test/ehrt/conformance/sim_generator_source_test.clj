(ns ehrt.conformance.sim-generator-source-test
  "Real-engine acceptance (SS-2 Step 3; ruling 3's own \"the real-engine
  paths are integration-tier\"): ehrt.corpus.interface
  ...
  not just synthea's own SS-1 golden-catalog trial. Runs unconditionally
  (ADR-0005): sim is an in-process mount now, never a sibling checkout
...
(deftest sim-generator-source-round-trips-through-intake-test
  (let [seed 101
        ...
        resolve-result (generator-source/resolve-generator-source! :sim {:seed seed :patients patients})]
    (is (result/ok? resolve-result)
        (str "generator-source/resolve-generator-source! :sim failed: " (pr-str resolve-result)))
