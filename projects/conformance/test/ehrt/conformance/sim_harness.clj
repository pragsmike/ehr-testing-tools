(ns ehrt.conformance.sim-harness
  "Test support (not `src/`): a thin project-local pass-through to
  `ehrt.corpus.interface/sim-run!` -- kept as its own namespace so this
  ...
  (:require [ehrt.corpus.interface :as sim]))

(defn run!
  "Delegates to ehrt.corpus.interface/sim-run! unchanged."
  [opts]
  (sim/sim-run! opts))
