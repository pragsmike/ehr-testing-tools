...
(defn- run-ehr-with-piped-stdin!
  "Spawns `bin/ehrt args...`, writing input-bytes to the child's own
...
(deftest ^:integration real-pipe-stdin-intake-test
  ...
  (let [messages "MSH|^~\\&|A\n\nMSH|^~\\&|B\n\n"
    ...
    (is (= 0 (:exit-code result))
        (str "bin/ehrt exited non-zero -- output: " (:stdout result)))
