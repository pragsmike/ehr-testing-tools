...
refuses a non-ASCII commit message -- the ASCII law (AR-RL2-5) moved
into the commit path, since `bin/post-push-verify` check 2 runs after
the push and can only report what
`.agents/rulings.md#R-amend-unpushed-message-only` forbids fixing
(`notes/ADRs.md` ADR-0157). The one config above installs all three;
...
`build-session` skill's VERIFICATION section has the full rule).

**`bin/ground-truth-bracket` is its sibling and is NOT a
regression-oracle claim** (ADR-0175 E1): the oracle hashes each root's
`{:ground-truth :hl7}` pair as ONE digest, so an emission-only change
moves every engine-layer root and says nothing about the half that must
not move. Same worktrees and `digest.clj`, `:ground-truth` only; an
emission sweep owes both lines -- bracket IDENTICAL, oracle DIFFERS declared.
...
**The ground-truth event log** (ADR-0141) — the artifact every emitter
renders from — has its Malli schema at `ehrt.sim-engine.event-schema`
(`components/sim-engine`), its committed EDN export at
`components/sim-engine/resources/sim-engine/event-schema.edn` (gated at
parity with the schema, regenerated by `make event-schema-export`), and
its reader-facing section in `docs/formats.md`. The version is carried
in the MANIFEST, key `:event-schema-version`
(`rulings.md#R-event-schema-versioned`). Named here because a cold read
of the `:onboarding` set could reach the four ADR-0141 rules and the
version key but no path, and inferred `components/provenance` — which
owns the manifest family, but not this schema (ADR-0147's cold-read
probe, Q5).
...
- [`.agents/state.md`](.agents/state.md) — the continuity register a new
  design session reads first: hand-owned judgement, watch items,
  environment, the design-channel contract, and a pointer row per
  register naming its gate. Capped at 120 lines and linted (ADR-0147).
  Its countable half — component graph, vendored inventory, gate roster,
  register sizes, reading-set actuals — is
  [`.agents/state-derived.md`](.agents/state-derived.md), **GENERATED**
  by `make docsgen` from the live tree and diffed by CI. Take any number
  from there, never from prose.
- [`.agents/session-records/`](.agents/session-records/README.md) —
  one dated record per non-trivial session, written as its last
  pre-push act (charter R-A, `notes/ADRs.md` ADR-0023). The README is
  the convention; `INDEX.md` beside it is the generated listing.
