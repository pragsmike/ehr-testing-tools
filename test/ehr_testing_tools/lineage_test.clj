(ns ehr-testing-tools.lineage-test
...
(defn- sample-fields []
  {:parent (digest/sha256-string "base-datum-bytes")
   :stage :mutate
   :transformation sample-transformation
   :produced (digest/sha256-string "mutant-datum-bytes")})
...
(deftest build-references-parent-and-produced-by-content-hash-test
  (let [fields (sample-fields)
    ...
    (is (= (digest/sha256-string "base-datum-bytes") (:parent r)))
    (is (= (digest/sha256-string "mutant-datum-bytes") (:produced r)))))
...
(deftest tampering-with-any-field-invalidates-the-content-hash-test
  ...
  (let [r (lineage/build (sample-fields))]
    (is (not (lineage/valid-content-hash? (assoc r :produced (digest/sha256-string "different")))))
...
(deftest different-parent-produces-a-different-id-test
  (let [r1 (lineage/build (sample-fields))
        r2 (lineage/build (assoc (sample-fields) :parent (digest/sha256-string "a different base")))]
    (is (not= (:id r1) (:id r2)))))
...
(deftest content-addressing-property-test
  (let [hash-gen (gen/fmap digest/sha256-string gen/string-ascii)
