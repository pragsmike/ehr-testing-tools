...
The stale-path gate gets every tracked doc surface: 25 dead links fixed, and two halves deliberately registered instead of improvised — repo review 3's second fix session (`.agents/plans/2026-08-15-repo-review-3-plan.md` Session B), on the arc ruling *"accept all."* D1-2's 25 dead markdown links all sat under `components/<x>/docs/`, invisible to a gate whose own docstring read *"Deliberately scoped: this scan covers docs/ ... only"* — and whose origin (P1-1, 2026-07-31) was this exact link family. A fourth scan now resolves markdown link destinations over every `*.md` under `docs/**` and `components/<x>/docs/**`, component doc roots enumerated from the filesystem rather than a hand-list, with a test asserting that; the retired sentence is replaced by a per-scan population statement naming all four scans and how each is enumerated. Red witnessed at **exactly 25** before any fix, remainder after the 19 mechanical re-depths **exactly 6** — both stated midpoints held, and every rewritten target was resolved individually rather than blind-rewritten. All four D1-8 exclusion classes encoded with their own tests, three of them structurally. **The 6 "gone-target" links were not a second class:** both targets exist, frozen — these docs came from the pre-merge `tools` repo, where `.agents/memory/patterns.md` and `.agents/plans/archive/judge-gate-refactor.md` were live siblings, and the merge froze that tree into `notes/tools/agents/`; the frozen file's numbered pattern 15, "Provenance is measured at the point of execution", is verbatim what all five citing sentences invoke. Ruled R-B1 *"Re-point all six"*, label text included. Two of the prompt's own design premises did not survive the tree and were **registered rather than improvised** (new rows D1-9, D1-10): the root-anchored *backticked* half cannot reach green under D1-8's four exclusions (216 dead, 95 even with component-root resolution, many inside `docs/` which D1-2 measured clean; the residue is dominated by an unnamed post-relocation basename-shorthand class, and contains real findings — retired `components/tools` cited twice in `docs/dev/architecture.md`), and widening the denylist families to `components/<x>/docs/` turns 15 more files red. Zero `src/` outside the one test namespace; vendored bytes verbatim
