...
(Patient, Encounter, Observation) — M6's `ehrt.sim.emit-state`
...
for the mechanism (`ehrt.sim.v2-replay`) and
...
Emitter landed at M6 (`ehrt.sim.emit-state`, R4 JSON) — a Bundle
