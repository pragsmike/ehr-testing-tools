> Provenance, frozen at merge: moved intact from `ehr-testing-tools`
> (origin repo, tagged `stable-pre-monorepo` at `f848b67`) into the
> `ehr-testing` workspace on 2026-07-28, byte-identical below except
> this header. Not updated for the workspace's new paths or
> `ehrt.tools.*`/`ehrt.palgebra.*` namespaces -- see `notes/ADRs.md`
> ADR-0002 for the workspace's own, current decision record, which
> cross-references entries here as `tools/ADR-NNNN`.

# Architecture / Authoring Decision Records — ehr-testing-tools
