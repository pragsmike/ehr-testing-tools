   ...
   [;; Artifact registry design: ADR-0005. --all introduced in D13,
    ...
    {:group "play"
     ...
     :flags [{:flag "--path" :doc "alternative to the positional PATH"}
             ...
             {:flag "--sink" :doc "a file: destination designator -- write the paced output (byte-identical to unpaced) there instead of showing the ticker. Message input only (:play-sink-unsupported-for-events on an event log, which has no wire framing to write). dir: and blaze: are recognized but deferred."}]}]})
