...
(defn- check-command
  ...
  (let [log (try (edn/read {:eof ::eof} (java.io.PushbackReader. *in*))
    ...
    (cond
      (= ::eof log) (sim/error :empty-input {:message "expected a ground-truth EDN vector on stdin"})
      (= ::unreadable log) (sim/error :unreadable-input {:message "stdin was not readable EDN"})
      (not (vector? log)) (sim/error :malformed-input {:message "expected a vector of event maps"})
      :else (sim/check-all log))))

(defn- unknown-action
  [action]
  (sim/error :unknown-command
             {:message (str "unknown sim action: " (pr-str action))
              :known (mapv :verb (:verbs help-group))}))

(defn- version-command
  "sim version: this library's version (`ehrt.sim.interface/version`,
  ...
  error (`ehrt.sim.interface/git-sha`'s own never-throwing
  contract)."
  [_opts]
  (sim/ok {:version sim/version :git-sha (sim/git-sha)}))
...
(defn dispatch-action
  ...
  ([action opts {:keys [run-fn check-fn identifiers-fn version-fn]
                 :or {run-fn sim/run-command
                      check-fn check-command
                      identifiers-fn sim/identifiers-command
...
(defn- bare-format-gate
  ...
  (cond
    ...
    (sim/rejected :format-er7-requires-emit-hl7
    ...
    (sim/rejected :format-ground-truth-requires-run
