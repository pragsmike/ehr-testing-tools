...
`ehr-testing-sim.run/run-command`, not just from a direct
...
case (`ehr-testing-sim.emit-hl7/pid-segment` applies uniformly). Note
OBX-8 `H` (abnormal-high) on the leukocyte and hemoglobin results —
computed truth from `ehr-testing-sim.order-profiles/abnormal-flag`,
