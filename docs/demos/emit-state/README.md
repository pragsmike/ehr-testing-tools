...
- [`fhir-bundle-patient1.json`](fhir-bundle-patient1.json) — patient
  1's own end-of-run FHIR Bundle (`--emit fhir`, `ehr-testing-sim.emit-state/bundle-run`):
...
```json
{
  ...
  "identifier": [{"system": "urn:ehr-testing-sim:mrn", "value": "MRN000001"}],
...
- **`Patient.id` ("PID-000000-918175ce") is `ehr-testing-sim.engine/patient-id-for`'s
  own internal id for patient ordinal 0** — never rendered on the wire
  itself (HL7 has no field for it), but the SAME id `ehr-testing-sim.emit-hl7`
...
- **The Observation's LOINC code (`6690-2`, Leukocytes) and abnormal
  ...
  from the identical `ehr-testing-sim.order-profiles/abnormal-flag`
