...
(`ehr-testing-sim.gmf-interpreter`). This is **script-space**: it
...
`ehr-testing-sim.engine/evolve` folding ground-truth events
...
`ehr-testing-sim.gmf-interpreter/step`/`walk-module`/`run-module`
never touch `ehr-testing-sim.engine/PatientState`, never call
...
**Persona** (seeded). At registration, `ehr-testing-sim.persona/persona`
...
queue is prepended with (`ehr-testing-sim.engine/run`), which is also
where script-space begins for any patient carrying a module assignment.

**The module walk, in two phases** (`ehr-testing-sim.gmf-interpreter/run-module`,
...
- **History phase** (DOB → registration instant): the walk fast-forwards
  ...
  pathway step (`ehr-testing-sim.compile-trajectory`'s own
...
**`CompileTrajectory`** (`ehr-testing-sim.compile-trajectory`) turns
...
steps at that point (`ehr-testing-sim.engine/run`'s own
...
**`InjectChurn`** (`ehr-testing-sim.churn`) is an IR→IR transform:
...
(`ehr-testing-sim.engine/pop-min`) — every patient's own queued steps,
...
same patient-state map (`ehr-testing-sim.facility/occupancy-board`) —
...
(`ehr-testing-sim.engine`'s own `:discharge` method, scanning `world`'s
...
yet a value `ehr-testing-sim.engine/PatientState`'s `:status` enum
actually carries.
