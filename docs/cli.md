...
Generate a deterministic synthetic Synthea corpus.

| Flag | Default | Meaning |
|---|---|---|
| `--config-path` | — | Synthea properties file |
| `--seed` | — | patient-generation seed (integer) |
| `--clinician-seed` | — | clinician-generation seed (integer) -- required: Synthea defaults this to wall-clock time otherwise, which breaks reproducibility even with --seed pinned |
| `--population` | — | population size (integer) |
| `--reference-date` | — | generation reference date, YYYYMMDD -- required: Synthea otherwise generates relative to wall-clock "now" |
| `--output-dir` | — | output directory for the corpus + manifest.edn |
| `--locale` | `en-US` | BCP47-ish locale |
| `--timezone` | `UTC` | timezone |
| `--java-bin` | `resolved via the artifact registry` | java executable to invoke |
| `--lockfile-path` | `artifacts.lock.edn` | path to the lockfile |

### `ehr corpus mutate`

Apply one mutation operator at one locator to every matching file under --input.

| Flag | Default | Meaning |
|---|---|---|
| `--input` | — | a file, or a directory of files sharing one locator's shape |
...
| `--output-dir` | — | directory for mutants + a lineage/ sidecar subdirectory |

### `ehr corpus intake`

Catalog a foreign (not generated by this repo) corpus batch.

| Flag | Default | Meaning |
|---|---|---|
| `--source-dir` | — | directory of files to catalog |
