...
a demo. Chapters 1–7 are landed — their promises below are firm.
...
**Chapter 8's own title and one-line promise is still a working
proposal** (channel-inferred, disclosed as such — Check's own topic
folds into Chapter 8 alongside verdict-reading at scale, since a
per-file assertion's own verdict is exactly what Chapter 8 already
teaches reading at scale), not yet ruled by name; a future session may
retitle it without reopening Chapters 1–7.
...
Chapter 3, and against each of Chapter 4 through Chapter 7's own
landing commit, respectively, for Chapters 4 through 7 — each
chapter's strips carry their own session's fresh witness, never a
stale carry-forward from an earlier chapter's own pass. Nothing here is regenerated by hand: if a command's real output
