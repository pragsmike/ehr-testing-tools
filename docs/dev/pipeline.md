<!-- GENERATED by `make pipeline` from components/tools/docs/pipeline.edn -- do not hand-edit.
...
The equations are the source of truth: [components/tools/docs/pipeline.edn](../../components/tools/docs/pipeline.edn). A stage marked `# planned` below is designed but not yet built -- its equation and law are fixed, its implementation isn't.
