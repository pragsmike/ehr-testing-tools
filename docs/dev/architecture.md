...
flowchart LR
    ...
    simtrajectory[patient-simulator]
    personsimulator[person-simulator]
    simemithl7[sim-emit-hl7]
    ...
    simtrajectory --> kernel
    personsimulator --> simmodel
    personsimulator --> simengine
    simemithl7 --> simmodel
    simemitfhir --> simengine
    simcheck --> simengine
    simcheck --> simmodel
    simcheck --> kernel
    sim --> kernel
...
| `components/provenance` | component | The provenance-manifest schema family: ManifestV0/V1/V1_1 and their `valid?`/`valid-v1?`/`valid-v1-1?` predicates (`ehrt.provenance.interface`), moved verbatim out of `components/corpus` (sim split B, M1, 2026-08-04, `.agents/plans/2026-08-04-sim-split-b-plan.md` AR-2). corpus → sim already exists (`ehrt.corpus.sim-adapter` requires the sim façade, ADR-0012), so sim → corpus for this schema would be a cycle — `provenance` is the only acyclic single home both depend on. Builders stay producer-side (corpus's `build`/`build-v1-1`, sim's `build`); this component holds no builder logic. Depends on nothing but malli — forbidden-forever from depending on corpus, sim, or anything else in the workspace. |
...
| `components/sim-engine` | component | The discrete-event simulation core (`decide`/`evolve`, the RNG-threaded run loop) plus its two catalytic config namespaces, InjectChurn (`churn`) and the order/result catalytic (`order-profiles`) (`ehrt.sim-engine.interface`). Extracted from `components/sim` (sim split B stage M2, 2026-08-04, `.agents/plans/2026-08-04-sim-split-b-plan.md`, `notes/ADRs.md` ADR-0043) — a pure move, no logic change; interface width (`run`/`config-keys`/`replay`/`documented-step-rejection-reasons`/`default-churn-profile`/`sample-profile`/`default-profiles`/`abnormal-flag`) derived the same grep-evidence way, residual sim's `run`/`check`/`emit-state`/`identifiers` confirmed the real external callers. Depends on `sim-model` and `patient-simulator`; forbidden-forever from depending on `components/sim` or anything corpus-derived. |
| `components/patient-simulator` | component | GMF module loading (`gmf`), interpretation (`gmf-interpreter`), and CompileTrajectory: clinical-trajectory → pathway IR (`ehrt.patient-simulator.interface`). Extracted from `components/sim` (sim split S2, 2026-08-02, same plan, `notes/ADRs.md` ADR-0025) — a pure move; interface width (`load-module`/`valid-modules-config?`/`run-module`/`compile-trajectory`) derived the same grep-evidence way, residual sim's `engine`/`run` confirmed the only two real external callers. Depends on `sim-model` and kernel only; forbidden-forever from depending on `components/sim` or anything corpus-derived. |
| `components/person-simulator` | component | The demographic and identity life-arc layer (`ehrt.person-simulator.interface`): bespoke hazard-rate processes producing a t-ascending stream of person events — residence moves, employment and coverage changes, households, pregnancy → delivery, occupational injury, death, and the identification dispositions — which the engine folds. Chartered by ADR-0172 (rulings A1 B1 C1 D1 E1 F1 G1) and gated by its own `docs/limitations.md`. Depends on `sim-model` and, for the stream-partition surface (`stream`/`newborn-id-tag`) ALONE, `sim-engine`; forbidden-forever from depending on `components/sim`, `components/patient-simulator`, either emitter, or anything corpus-derived, and no `sim-engine` namespace may depend on IT (ADR-0172 limitations row 10, gated). **Arc 2b landed it ALONE (ruling F1): nothing in the workspace calls it yet, which is what made the corpus provably untouched.** |
| `components/sim-emit-hl7` | component | The HL7v2 emitter (`emit-hl7`), the v2-replay wire-side accumulator, and site profiles (`ehrt.sim-emit-hl7.interface`). Extracted from `components/sim` (sim split S3 / GMF coverage Wave D stage D0, 2026-08-02, `.agents/plans/2026-08-02-sim-split-plan.md`, `notes/ADRs.md` ADR-0029) — a pure move; interface width (`emit`/`control-id-for`/`default-reference-date`/`default-utc-offset`) derived the same grep-evidence way, residual sim's `identifiers`/`run` confirmed the only two real external callers — `v2-replay` and `site-profile` have no real external caller at all and stay fully internal. Depends on `sim-model` only; forbidden-forever from depending on `components/sim`, `components/patient-simulator`, or anything corpus-derived. |
| `components/sim-emit-fhir` | component | The state-based FHIR R4 Bundle emitter (`ehrt.sim-emit-fhir.interface`) — `sim-emit-hl7`'s own sibling as a rendering accent, but NOT same-shaped: it folds the ground-truth log via `ehrt.sim-engine.interface/replay` and renders from a snapshot of folded state, where `sim-emit-hl7` renders per-event from the log directly. Extracted from `components/sim` (sim split B stage M3, 2026-08-04, `.agents/plans/2026-08-04-sim-split-b-plan.md`, `notes/ADRs.md` ADR-0043) — a pure move plus the AR-3 namespace rename (EmitState's own former namespace becomes `ehrt.sim-emit-fhir.emit-fhir`); interface width (`bundle-run` only) derived the same grep-evidence way, residual sim's `run`/`identifiers` confirmed the only two real external callers — `snapshot-at` has no real external caller and stays fully internal. Depends on `sim-engine` only; forbidden-forever from depending on `components/sim`, `components/sim-model`, `components/patient-simulator`, or anything corpus-derived. |
