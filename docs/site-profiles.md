...
2. **Code-table overrides.** A site profile supplies its own value set
   ...
   **None of those table numbers is a citation this
   ...
   `ehrt.sim-emit-hl7.site-profile`'s standard tables is this
...
3. **Naming: `:surge-format` migrates to the profile.** A config-level
   ...
   TRANSFORM a caller applies BEFORE `ehrt.sim-engine.run/run`, never
...
`ehrt.sim-engine.config/config-keys`, so it is structurally incapable
